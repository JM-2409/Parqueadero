...
export function SuccessMessage({ message }: { message: React.ReactNode }) {
  ...
  return (
    <div className={`mb-6 p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl flex items-center gap-2 transition-opacity duration-300 ${visibleClass}`}>
      <CheckCircle2 size={20} className="text-emerald-500 flex-shrink-0" />
      <div className="flex-1 overflow-hidden break-words text-sm font-medium">{message}</div>
    </div>
  );
}
