...
export default function TariffSettings({
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const existingVehicleTariffs = tariffs.filter(
      (t) => t.vehicle_type === vehicleType,
    );

    if (existingVehicleTariffs.some((t) => t.rate_type === rateType)) {
      alert(
        `Ya existe una tarifa de tipo "${RATE_LABELS[rateType] || rateType}" para ${vehicleType}. Elimínala primero si deseas actualizarla.`,
      );
      return;
    }
    ...
    if (error) {
      ...
      if (
        typeof error === "object" &&
        JSON.stringify(error).includes("start_time")
      ) {
        setErrorMsg(
          "Faltan columnas. Ejecuta el nuevo SQL para tariffs_v3 en el README.",
        );
      } else {
        setErrorMsg(
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Configura las reglas de cobro por vehículo. El sistema aplica la
      ...
      {allowedVehicles.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 bg-white rounded-3xl shadow-md border border-slate-100 text-center">
          ...
          <p className="text-slate-500 max-w-sm">
            Primero debes configurar los tipos de vehículos permitidos (Carro,
            Moto, etc.) en la pestaña de Configuración.
          </p>
        </div>
      ) : (
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {allowedVehicles.map((vehicle) => {
              const vehicleTariffs = tariffs.filter(
                (t) => t.vehicle_type === vehicle,
              );
                    ...
                    {vehicleTariffs.map((t) => (
                      ...
                      >
                        <div>
                          ...
                          {(t.rate_type === "dia" || t.rate_type === "noche") &&
                            t.start_time && (
            ...
            {tariffs.length === 0 && (
              <div className="flex flex-col items-center justify-center p-12 bg-white rounded-3xl border border-dashed border-slate-200 text-center h-full min-h-[300px]">
                ...
                <p className="text-sm text-slate-400 mt-1">
                  Crea tu primera tarifa usando el formulario.
                </p>
              </div>
            )}
          </div>
