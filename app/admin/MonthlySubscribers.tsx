  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Abonados Mensuales</h2>
          <p className="text-sm text-slate-500 mt-1">Gestiona los clientes con mensualidad y su periodo de validez.</p>
        </div>
      ...
      {error && <div className="p-4 bg-red-50 text-red-600 border border-red-200 rounded-2xl text-sm font-medium">{error}</div>}
      {success && <div className="p-4 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-2xl text-sm font-medium">{success}</div>}

      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 border border-slate-100 shadow-sm rounded-3xl bg-white p-6 sticky top-24 h-max">
          ...
          <form onSubmit={handleAdd} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input 
                  ...
                  value={newSub.owner_document} 
                  onChange={(e) => setNewSub({...newSub, owner_document: e.target.value})}
                  className="w-full bg-slate-50 border-0 text-slate-900 text-sm rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                  placeholder="CC / NIT"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Celular</label>
                <input 
                  type="text" 
                  value={newSub.phone} 
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Tipo de Veh.</label>
                <div className="relative">
                  <select
                    value={newSub.vehicle_type}
                    onChange={(e) => setNewSub({...newSub, vehicle_type: e.target.value})}
                    className="w-full bg-slate-50 border-0 text-slate-900 text-sm rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-indigo-500 appearance-none outline-none font-medium transition-all"
                  >
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border-0 text-slate-900 text-sm rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none font-bold transition-all"
                  min="0"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 ml-1">Fecha Inicio</label>
                <input 
                  type="date" 
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 ml-1">Vencimiento</label>
                <input 
                  type="date" 
            ...
            <button 
              ...
              className="w-full mt-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-4 py-3.5 font-bold transition-all shadow-sm shadow-indigo-200 flex items-center justify-center gap-2 disabled:opacity-70"
            >
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="border border-slate-100 rounded-3xl bg-white overflow-hidden flex flex-col shadow-sm">
            <div className="p-6 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
              ...
              <div className="relative w-full sm:w-72">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border-0 text-slate-900 text-sm rounded-xl pl-10 pr-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-0 sm:p-6 bg-slate-50/50">
              {loading ? (
                <div className="flex flex-col items-center justify-center py-12"><Spinner className="text-indigo-500 mb-2" /><p className="text-slate-500 text-sm">Cargando...</p></div>
                ...
                <div className="overflow-x-auto bg-white sm:rounded-2xl border-y sm:border border-slate-100 shadow-sm">
                  <table className="w-full text-left text-sm border-collapse">
                    <thead>
                      <tr className="bg-slate-50/80 border-b border-slate-100 text-slate-500 text-[10px] uppercase tracking-wider">
                        ...
                        <th className="py-4 px-5 font-bold text-center">Acciones</th>
                      </tr>
                    </thead>
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                          <td className="py-4 px-5">
                            <span className="font-mono font-bold text-slate-900 bg-slate-100 px-2.5 py-1 rounded-md inline-block">{item.plate}</span>
                            <div className="text-[10px] font-bold text-slate-400 mt-1.5 uppercase tracking-widest">{item.vehicle_type}</div>
                          </td>
                          <td className="py-4 px-5">
                            <div className="font-bold text-slate-900">{item.owner_name}</div>
                            {item.phone && <div className="text-xs font-medium text-slate-500 mt-1">{item.phone}</div>}
                          </td>
                          <td className="py-4 px-5">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={isExpired ? "text-red-600 font-bold text-xs bg-red-50 px-2 py-1 rounded-md" : "text-emerald-700 font-bold text-xs bg-emerald-50 px-2 py-1 rounded-md"}>
                          ...
                          <td className="py-4 px-5 text-center">
                            <button 
                              ...
                              className="text-slate-400 hover:text-red-600 w-8 h-8 flex items-center justify-center mx-auto hover:bg-white border hover:border-red-100 border-transparent rounded-lg transition-all opacity-0 group-hover:opacity-100"
