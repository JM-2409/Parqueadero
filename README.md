...
- **Fix de Seguridad en Inicialización de Usuarios**: Se actualizó el flujo de creación para que, si el sistema está vacío (sin superadmins), solo se permita crear y registrar estrictamente un usuario con el rol de `superadmin`. Se bloquea la creación no autenticada de cualquier otro rol.
...
- **Exportación a Excel (XLSX)**: Se integró la librería `xlsx` para permitir a los empleados exportar el historial de registros a un documento en formato Microsoft Excel de un solo click.
