...
- **Borrado de Registros Defectuosos**: Se añadió temporalmente el botón de eliminación de registros permanente ("Borrar Registro" con el ícono rojo de bote de basura en Detalles de Acciones) para poder solucionar el problema de autos atrapados.
- **Formato Inteligente de Recibos y Exportación**: Los campos estáticos (Marca, Color, Propietario) han sido removidos de los bloques desplegables del historial y delegados directamente a los datos extra dinámicos `custom_fields_data` para reportes precisos.
- **Exportados en CSV Completos**: La exportación ahora lee las columnas dinámicas de la configuración de la sucursal y extrae `custom_fields_data` evitando vacíos en el Excel, proveyendo al contable una grilla adaptativa.
- **Impresión Simple y Optimizada**: El modal de recibo fue optimizado estructuralmente para evitar forzar saltos de página y asegurar que el ticket o factura salgan en una sola hoja al hacer uso del diálogo de impresión en Chrome/Safari.
- **Entrada Manual Agilizada a (24H)**: En la pestaña de historial manual, los campos de hora fueron transicionados a texto estricto 24H (`14:50`, `08:20`) para maximizar la velocidad de tipificación en los migradores de datos y digitadores de históricos del parqueadero.
