...
export default function AdminDeviceManagement({ parkingLotId }: { parkingLotId: string }) {
  ...
  const fetchDevices = async () => {
    try {
      setLoading(true);
      // Fetch devices for employees in this parking lot
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 p-8">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Equipos de Vigilantes</h2>
          <p className="text-slate-500">Gestiona el acceso de los equipos de los empleados de este parqueadero.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-slate-500 text-center p-8">No hay equipos registrados para empleados.</p>
      ) : (
